...
async def join_server(ctx, target_server_id: str):
    """Join 5 valid users per run with 60s delay between each (5 × 60s = 5 minutes total)."""
    ALLOWED_CHANNEL_ID = 1514837297810509895  # only works in this channel
    CHECK_LIMIT        = 1000                 # max users read from auths.txt
    JOIN_LIMIT         = 5                    # members joined per !djoin run
    VALIDATE_CHUNK     = 20                   # concurrent token-validation batch size
    JOIN_DELAY         = 60                   # seconds between each join (5 × 60 = 5 min)
    ...
    try:
        ...
        def build_waiting_embed(next_num, total_joining, success, failed, refreshed, secs_left):
            e = discord.Embed(
                title="JOIN IN PROGRESS",
                description=(
                    f"Server: **{server_name}**\n"
                    f"Role Limit: **{total_joining} users**\n"
                    f"Success: **{success}** | Failed: **{failed}** | Invalid: **{invalid_count}**\n"
                    f"Progress: **{next_num - 1}/{total_joining}**\n"
                    f"⏳ Next join in: **{secs_left}s**"
                ),
                color=0xFEE75C
            )
            return e
        ...
        def build_ratelimit_embed(joined, total_valid, success, failed, secs_left):
            e = discord.Embed(
                title="⚠️ RATE LIMITED — Process paused",
                description="Discord returned 429. Waiting before retrying...",
                color=0xED4245
            )
            e.add_field(name="🏠 Server",          value=server_name,                   inline=False)
        ...
        def build_done_embed():
            cmd_user  = ctx.author.mention
            user_role = ctx.author.top_role.name if hasattr(ctx.author, 'top_role') and ctx.author.top_role else "None"

            # Build User Status field (show first 15, then "… and X more")
            SHOW = 15
            lines = [f"<@{uid}> {st}" for uid, st in user_status_list[:SHOW]]
            remaining = len(user_status_list) - SHOW
            status_text = "\n".join(lines)
            if remaining > 0:
                status_text += f"\n... and {remaining} more"

            e = discord.Embed(
                title="JOIN COMPLETED - LIMIT REACHED",
                color=0x57F287 if success_count > 0 else 0xED4245
            )
            e.add_field(name="Server",              value=server_name,                          inline=False)
            e.add_field(name="Command User",        value=cmd_user,                             inline=True)
            e.add_field(name="User Role",           value=user_role,                            inline=True)
            e.add_field(name="Max Allowed",         value=f"{JOIN_LIMIT} users",                inline=True)
            e.add_field(name="Successfully Added",  value=success_count,                        inline=True)
            e.add_field(name="Already in Server",   value=already_in_count,                     inline=True)
            e.add_field(name="Failed Joins",        value=failed_count,                         inline=True)
            e.add_field(name="Invalid Tokens",      value=invalid_count,                        inline=True)
            e.add_field(name="Total Checked",       value=total_read,                           inline=True)
            e.add_field(name="Role Limit Used",     value=f"{success_count}/{JOIN_LIMIT}",       inline=True)
            if status_text:
                e.add_field(name="User Status",     value=status_text,                          inline=False)
            return e
        ...
        invalid_count   = 0
        checked         = 0
        user_status_list = []   # (user_id, "-> status") for every token processed
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i in range(0, total_read, VALIDATE_CHUNK):
                ...
                for idx, res in enumerate(results):
                    ...
                    if isinstance(res, Exception) or res[1] is None:
                        invalid_count += 1
                        uid = chunk[idx]['user_id']
                        user_status_list.append((uid, "-> Invalid token"))
                        print(f"❌ Invalid token: {uid}")
                    else:
            ...
            if total_valid == 0:
                await status_msg.edit(embed=discord.Embed(
                    title="😔 No valid tokens found",
                    description=f"{invalid_count} tokens are invalid/expired. Ask users to re-authenticate.",
                    color=0xED4245
                ))
                return

            # ═══════════════════════════════════════════════════════════════════
            # PHASE 2 — Join only JOIN_LIMIT valid users, 60s delay between each
            #           5 users × 60s = exactly 5 minutes per !djoin run
            # ═══════════════════════════════════════════════════════════════════
            batch_to_join = valid_users[:JOIN_LIMIT]   # take only first 5 valid users
            total_joining = len(batch_to_join)
            await status_msg.edit(embed=build_join_embed(0, total_joining, 0, 0, 0))
            ...
            for idx, user in enumerate(batch_to_join):
                user_id     = user['user_id']
                valid_token = user['valid_token']
                if user['refreshed']:
                    token_refreshed += 1

                # ── 60s countdown BEFORE every join (including the first) ──────
                print(f"⏳ Waiting {JOIN_DELAY}s before joining user {idx+1}/{total_joining}...")
                for secs_left in range(JOIN_DELAY, 0, -10):
                    await status_msg.edit(embed=build_waiting_embed(
                        idx + 1, total_joining, success_count, failed_count, token_refreshed, secs_left
                    ))
                    await asyncio.sleep(min(10, secs_left))
                ...
                try:
                    ...
                    async with session.put(join_url, headers=bot_headers, json={"access_token": valid_token}) as resp:
                        ...
                        if resp.status == 429:
                            print(f"⚠️ 429 hit at user {idx+1}/{total_joining} — pausing {RATE_LIMIT_PAUSE}s")
                            for secs_left in range(RATE_LIMIT_PAUSE, 0, -5):
                                await status_msg.edit(embed=build_ratelimit_embed(
                                    idx + 1, total_joining, success_count, failed_count, secs_left
                                ))
                        ...
                        if resp_status == 201:
                            success_count += 1
                            user_status_list.append((user_id, "-> Joined ✅"))
                            ...
                            already_in_count += 1
                            user_status_list.append((user_id, "-> Already in server"))
                            print(f"⚠️ Already in server: {user_id}")
                        else:
                            failed_count += 1
                            user_status_list.append((user_id, f"-> HTTP {resp_status}"))
                            print(f"❌ Join failed {user_id}: {resp_status}")

                except Exception as e:
                    failed_count += 1
                    user_status_list.append((user_id, "-> Error"))
                    print(f"❌ Exception {user_id}: {e}")

                # Update embed to show this user was processed
                await status_msg.edit(embed=build_join_embed(
                    idx + 1, total_joining, success_count, failed_count, token_refreshed
                ))
